...
def detect_dial(img):
    ...
    img_binary = create_binary(img_orig)  # Create a binary for the contour detection
    img_dial, mask = find_contours_binary(img_binary, img_output)

    mask_dial, mask_cap, x_cap, y_cap, r_dial = find_dial(img_orig, img_binary, mask)
...
def create_binary(img):
    ...
    img_hls = cv2.cvtColor(img, cv2.COLOR_BGR2HLS)  # Convert to Hue, Saturation, Lightness
    channels = cv2.split(img_hls)  # extract the lightness channel

    img_gray_gamma = adaptive_gamma_correction.agc_1c(img_gray)  # Apply adaptive gamma correction

    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
    img_lightness = clahe.apply(channels[1])  # Separate the lightness values

    ret, mask_lightness = cv2.threshold(img_lightness, np.mean(img_lightness), 1, cv2.THRESH_BINARY)

    img_gray_equalized = img_gray_gamma * mask_lightness  # Mask gray image based on the lightness
...
def find_dial(img_dial, img_binary, mask):

    img_output = img_dial.copy()
    mask_value = (255 - img_binary) * mask
    mask_value, x, y, r = find_contours_cap(mask_value)
...
def find_contours_cap(img_binary):
