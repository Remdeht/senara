...
def get_red_pointer(img, mask_cap):
    ...
    image_blurred = cv2.GaussianBlur(image_corrected, (9, 9), 1)  # Blur for noise removal

    image_HSV = cv2.cvtColor(image_blurred, cv2.COLOR_BGR2HSV)  # Convert to the Hue Color space
    ...
    channels = cv2.split(img_hls)
    lightness = adaptive_gamma_correction.agc_1c(channels[1])  # Lightness comes in handy to remove possible noise
    ...
    mask_lightness = cv2.inRange(lightness, np.mean(lightness)-(.5 * np.std(lightness)), np.max(lightness))

    if np.count_nonzero(mask_saturation) < 100:  # Safety clause in case saturation are low because of shade
        mask_saturation = cv2.inRange(saturation, np.mean(saturation) + (2.5 * np.std(saturation)), np.max(saturation))

    mask_cap = (255 - mask_cap)
    total_mask = mask_cap * mask_saturation * mask_lightness

    return total_mask


def get_center_pointer(img, mask):
    """Main Function that binds the whole extraction of the red pointer togetether"""

    mask_pointer = get_red_pointer(img, mask)
    ...
    # Now we need to get the center of mass of the biggest contour

    mask_box_1 = create_mask_box(box1, mask_pointer)
