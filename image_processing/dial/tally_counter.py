# Extracts tally counter digits and performs classification.
# Libraries
import numpy as np
import cv2
import math
...
from .digit_classification import classify


def extract_counter(img, fd, n, flag_knn, flag_templates, name=None):
    ...
    box = find_tally_contour(img_counter, tally_height,
                             tally_width, name)  # Search for the contour of the tally counter based on Value & Hue
    ...
    if name is not None:
        ...
        img_counter_copy = img_counter.copy()
        cv2.drawContours(img_counter_copy, [box], -1, (0, 0, 255), 3)

        if not os.path.exists(ROOT_DIR + '\\results\\image_processing\\tally_counter'):
            os.makedirs(ROOT_DIR + '\\results\\image_processing\\tally_counter\\area_of_interest\\')
            os.makedirs(ROOT_DIR + '\\results\\image_processing\\tally_counter\\binary\\')
            os.makedirs(ROOT_DIR + '\\results\\image_processing\\tally_counter\\digits\\')
            os.makedirs(ROOT_DIR + '\\results\\image_processing\\tally_counter\\detected\\')

        cv2.imwrite(ROOT_DIR + '\\results\\image_processing\\tally_counter\\area_of_interest\\' + name, img_counter)
        cv2.imwrite(ROOT_DIR + '\\results\\image_processing\\tally_counter\\detected\\' + name, img_counter_copy)
    ...
    if side_1 >= side_2:
        ...
        num1, num2, num3, num4, num5, num6 = divide_tally_counter(box_transformed, rotated_img, name)
...
def find_tally_contour(img_counter, tally_height, tally_width, name=None):
    ...
    if name is not None:
        cv2.imwrite(ROOT_DIR + '\\results\\image_processing\\tally_counter\\binary\\' + name, img_binary_counter)

    expected_tally_area = tally_width * tally_height
    contour_area, cnt = get_biggest_contour(img_binary_counter)
    if cnt is None:
        return None
    # Following checks the contour area, when too small function returns a 0, when too large a 1
    cnt = check_contour(contour_area, cnt, expected_tally_area)

    # If the region is too small or too big the regions are dilated or eroded trying to get a suited region.
    # this is especially useful when the tally counter is split by the handle of the cap.
    ...
    # A rectangle is fitted around the contour, as the tally counter is rectangular
    box = general_functions.fit_rectangle(cnt)
    box = general_functions.order_rectangle_coordinates(box.tolist())
    # height of box is compared too the accepted tally height
    height_difference = int(abs(general_functions.distance(box[0], box[3]) - tally_height))

    if height_difference > tally_height * .1:  # If the contour is too high it is cropped.
    ...
    # same process only for the width now
    ...
    return box  # the box represents the rectangle encompassing the tally counter


def check_contour(area, cnt, expected_tally_area):
    """This method cheks if the size of the region is acceptable, in case the size is too small it return a 0
     and if its too large it return a 1"""
...
def get_biggest_contour(img_binary):
    """Gets the contour with the largest area from a set of contours"""
...
def opening(img_binary_counter, expected_tally_area):
    """Performs erosion on the image region in order to oprimize it's shape"""
...
def closing(img_binary_counter, expected_tally_area):
    """Performs dilation on the image region in order to oprimize it's shape"""
...
def divide_tally_counter(box_transformed, rotated, name=None):
    ...
    if name is not None:
        ...
        pattern = r"(.*).[jJ][pP][gG]"
        name = re.match(pattern, name)[1]

        if not os.path.exists(ROOT_DIR + '\\results\\image_processing\\tally_counter\\digits\\%s\\' % name):
            os.makedirs(ROOT_DIR + '\\results\\image_processing\\tally_counter\\digits\\%s\\' % name)

        cv2.imwrite(ROOT_DIR + '\\results\\image_processing\\tally_counter\\digits\\%s\\slot_1.jpg' % name, num1)
        cv2.imwrite(ROOT_DIR + '\\results\\image_processing\\tally_counter\\digits\\%s\\slot_2.jpg' % name, num2)
        cv2.imwrite(ROOT_DIR + '\\results\\image_processing\\tally_counter\\digits\\%s\\slot_3.jpg' % name, num3)
        cv2.imwrite(ROOT_DIR + '\\results\\image_processing\\tally_counter\\digits\\%s\\slot_4.jpg' % name, num4)
        cv2.imwrite(ROOT_DIR + '\\results\\image_processing\\tally_counter\\digits\\%s\\slot_5.jpg' % name, num5)
        cv2.imwrite(ROOT_DIR + '\\results\\image_processing\\tally_counter\\digits\\%s\\slot_6.jpg' % name, num6)

    return num1, num2, num3, num4, num5, num6


def recalculate_coordinates(original_image, point, rotation):
    """ Recalculates the coordinates as to the center of the image based on the rotation"""
