...
INNER_THRESHOLD = .40  # Sets the area of interest for the pointer detection
OUTER_THRESHOLD = .55
BREAKPOINT = float("%.2f" % (INNER_THRESHOLD + ((OUTER_THRESHOLD - INNER_THRESHOLD) / 2)))  # Middle of the aoi


def read_pointer(img_dial, zero_point, name=None):
    """ Main function that segments the pointer region and calculates it's value"""

    mask_pointer = create_mask_pointer(img_dial)
    binary_pointer, img_pointer = create_binary(img_dial, mask_pointer)

    contours, _ = cv2.findContours(binary_pointer, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    ...
    if possible_pointer_contours is None:
        pointer = alternative_process(img_dial, mask_pointer, name)  # alternative process for pointer detection
        ...
        if pointer is None:
            pointer = alternative_process(img_dial, mask_pointer, name)
    ...
    value = determine_pointer_value((cx, cy), (X, Y), zero_point)

    result_img = img_dial.copy()
    w, h = img_dial.shape[:2]
    mask = np.zeros((w, h), np.uint8)
    cv2.drawContours(mask, [pointer], -1, 255, -1)

    result_img[mask == 255] = 255

    if name is not None:
        if not os.path.exists(ROOT_DIR + '\\results\\image_processing\\pointer'):
            os.makedirs(ROOT_DIR + '\\results\\image_processing\\pointer\\binary\\')
            os.makedirs(ROOT_DIR + '\\results\\image_processing\\pointer\\binary_alternative\\')
            os.makedirs(ROOT_DIR + '\\results\\image_processing\\pointer\\pointer\\')

        cv2.imwrite(ROOT_DIR + '\\results\\image_processing\\pointer\\binary\\' + name, binary_pointer)
        cv2.imwrite(ROOT_DIR + '\\results\\image_processing\\pointer\\pointer\\' + name, result_img)

    return value


def alternative_process(img_dial, mask_pointer, name=None):
    """Alternative thresholding process in case the first one fails"""

    binary = create_binary_yuv(img_dial, mask_pointer)

    if name is not None:
        cv2.imwrite(ROOT_DIR + '\\results\\image_processing\\pointer\\binary_alternative\\' + name, binary)
...
def determine_pointer_value(pointer, center, zero):
    """Calculates the pointer value based on the angle between the pointer and the zero point"""
...
def create_binary_yuv(img, mask):
    """Creates a binaru image via image segmentation"""
...
def check_possible_pointers(contours, H, W):
    """Takes the three largest contours and checks if they are intersecting the edge of the AOI"""
...
def check_possible_pointers_yuv(contours, H, W):
    """Takes the five largest contours and checks if they are intersecting the edge of the AOI"""
...
def check_pixels_on_edge(cnt, H, W):
    """Function that counts amount of pixels located on the inner edge of the AOI"""
...
def create_binary(img, mask):
    """Creates binary image via image thresholding in different color spaces"""
...
def determine_final_pointer(contours, img, estimated_max_pointersize, img_org):
    """Final check to determine which region is the most liekly pointer region"""
    possible_pointers = []
...
def determine_best_candidate(pointers, img):
    """In case multiple regions are deemed to be suitable pointer regions the region with the highest redness value
    is chosen as the most likely pointer"""
    dic = {}
...
def create_mask_pointer(img_dial):
    """Creates a ring around the center of the dial in which the pointer is expected to be"""
    ...
    box = np.array(
        general_functions.order_rectangle_coordinates(box))  # Remove part of the dial that has no measurements anyway
...
def create_mask_inner(img):
    """Creates a mask of the inner half of the aoi"""
    height, width = img.shape[:2]
...
def create_mask_edge(height, width):
    """Creates a mask of the inner edge of the aoi"""
    center = (int(width / 2), int(height / 2))
    r = int(width / 2)
...
def create_mask_outer(img):
    """Creates a mask of the outer half of the aoi"""
    height, width = img.shape[:2]
...
if __name__ == '__main__':
    ...
    for water_meter in water_meters:
        print(water_meter)
        image_loc = str(DATA_DIR.joinpath(water_meter))

        img_dial = cv2.imread(image_loc)
        read_pointer(img_dial, (0, 0))
